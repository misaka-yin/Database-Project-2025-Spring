...
document.getElementById('navToggle').addEventListener('click', () => {
  ...
  loginForm.addEventListener('submit', async e => {
    ...
    const password = e.target.loginPassword.value;   // 改名 password
    try {
      // 假设后端登录接口返回 { id, firstName, lastName, role }
  ...
  registerForm.addEventListener('submit', async e => {
    e.preventDefault();
    // 拆分 firstName / lastName
    const data = {
      firstName: e.target.regFName.value,
      lastName:  e.target.regLName.value,
      email:     e.target.regEmail.value,
      password:  e.target.regPassword.value,
      // 如果你在表单里放了 <select name="role"> 或 hidden input:
      role:      e.target.role?.value || 'user'
    };
  
    // 简单校验两次密码一致
    if (data.password !== e.target.regConfirm.value) {
  ...
  function onLoginSuccess(user) {
    ...
    // 如果是管理员，显示 Admin Panel 链接
    if (user.role === 'admin') {
      const nav = document.querySelector('.header__nav .nav__list');
      nav.insertAdjacentHTML('beforeend',
        `<li><a href="admin.html" class="nav__link">Admin Panel</a></li>`
      );
    }
  }
